...
export function RuedaAlquimia() {
  ...
  return (
    <div className="panel-pergamino mx-auto w-fit p-10">
      <MarcoOrnamental />
      <h3 className="mb-6 text-center font-display text-3xl text-oro">Astrolabio de Alquimia</h3>

      <div className="relative mx-auto" style={{ width: RADIO * 2 + 80, height: RADIO * 2 + 80 }}>
        <div className="absolute inset-0 rounded-full border-2 border-bronce/60" />
        <div className="absolute inset-4 rounded-full border border-dashed border-bronce/25" />
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="font-technical text-[0.65rem] uppercase tracking-[0.15em] text-bronce/40">
            Piedras dragón
          </span>
        </div>
        ...
        {piedras.map((piedra, i) => {
          ...
          return (
            <button
              ...
              className="absolute left-1/2 top-1/2 -ml-8 -mt-8 flex h-16 w-16 flex-col items-center justify-center rounded-full border-2 border-bronce bg-tinta text-center shadow-[0_2px_10px_rgba(0,0,0,0.4)] transition-all duration-200 hover:border-oro hover:shadow-[0_0_10px_2px_rgba(201,162,39,0.45)]"
            >
              <span className="font-technical text-[0.6rem] text-bronce">{piedra.nombre}</span>
              {seleccion ? (
                <span className="font-mono text-[0.55rem] text-sello">{seleccion.grado.grado}</span>
              ) : (
